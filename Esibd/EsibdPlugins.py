""" This module contains only :class:`plugins<Esibd.EsibdPlugins.Plugin>` and plugin templates.
...
from Esibd.EsibdCore import INOUT, Parameter, PluginManager, parameterDict, DynamicNp, PRINT, Channel, MetaChannel
if sys.platform == 'win32':
    import win32com.client
aeval = Interpreter()

def providePlugins():
    """Returns list of :class:`plugins<Esibd.EsibdPlugins.Plugin>` that are available for activation in the :class:`~Esibd.EsibdCore.PluginManager` user interface accessible from :ref:`sec:settings`.

    :return: Plugin list
    :rtype: [:class:`~Esibd.EsibdPlugins.Plugin`]
    """
...
class Plugin(QWidget):
    ...
    All plugins are ultimately derived from the :class:`~Esibd.EsibdPlugins.Plugin` class.
    ...
    """File extensions that are supported by this plugin. If a corresponding
       file is selected in the :meth:`~Esibd.EsibdPlugins.Explorer`, the plugins :meth:`~Esibd.EsibdPlugins.Plugin.loadData` function will be called."""
    ...
    """Defines if the user can deactivate the plugin in the :class:`~Esibd.EsibdCore.PluginManager` user interface."""
    ...
    titleBar : QToolBar
    """Actions can be added to the titleBar using :meth:`~Esibd.EsibdPlugins.Plugin.addAction` or :meth:`~Esibd.EsibdPlugins.Plugin.addStateAction`."""
    ...
    """A reference to the central :class:`~Esibd.EsibdCore.PluginManager`."""
    ...
    axes : [mpl.axes.Axes]
    """The axes of :attr:`~Esibd.EsibdPlugins.Plugin.fig`."""
    initializedGUI : bool
    ...
    """A flag signaling if the plugin :attr:`~Esibd.EsibdPlugins.Plugin.dock` has been initialized.
       You may want to ignore certain events before initialization is complete."""
    ...
    def print(self, message, flag=PRINT.MESSAGE):
        ...
        :type message: str
        :param flag: Flag used to adjust message display, defaults to :attr:`~Esibd.EsibdCore.PRINT.MESSAGE`
    ...
    def test(self):
        """Runs :meth:`~Esibd.EsibdPlugins.Plugin.runTestParallel` in parallel thread."""
    ...
    def finalizeInit(self, aboutFunc=None):
        ...
        that modifies other :class:`Plugins<Esibd.EsibdPlugins.Plugin>`, e.g. adding an :class:`~Esibd.EsibdCore.Action` to the :class:`~Esibd.EsibdPlugins.DeviceManager`.

        :param aboutFunc: Function displaying the about dialog of the plugin, defaults to None.
            :meth:`~Esibd.EsibdPlugins.Plugin.about` is used if no other is provided.
    ...
    def provideDock(self):
        """Adding existing :attr:`~Esibd.EsibdPlugins.Plugin.dock` to UI."""
    ...
    def raiseDock(self, _show=True):
        """Raises :attr:`dock<Esibd.EsibdPlugins.Plugin.dock>` if _show is True."""
        if _show:
    ...
    def addAction(self, func=None, toolTip='', icon=None, before=None):
        ...
        :type icon: :class:`~Esibd.EsibdCore.BetterIcon`, optional
        ...
        :type before: :class:`~Esibd.EsibdCore.Action`, optional
        :return: The new Action
        :rtype: :class:`~Esibd.EsibdCore.Action`
        """
    ...
    def addStateAction(self, func=None, toolTipFalse='', iconFalse=None, toolTipTrue='', iconTrue=None, before=None, attr=None, restore=True):
        ...
        :type iconFalse: :class:`~Esibd.EsibdCore.BetterIcon`, optional
        ...
        :type iconTrue: :class:`~Esibd.EsibdCore.BetterIcon`, optional
        ...
        :rtype: :class:`~Esibd.EsibdCore.StateAction`
    ...
    def toggleTitleBar(self):
        """Adjusts the title bar layout and :attr:`~Esibd.EsibdPlugins.Plugin.titleBarLabel` depending on the state of the :attr:`~Esibd.EsibdPlugins.Plugin.dock` (tabbed, floating, ...).
    ...
    def loadData(self, file, _show=True):
        ...
        This should only be called for files where :meth:`~Esibd.EsibdPlugins.Plugin.supportsFile` returns True.
...
, True)
    ...
    def makeFigureCanvasWithToolbar(self, figure):
        """Creates :meth:`~Esibd.EsibdPlugins.Plugin.canvas`, which can be added to the user interface, and
        adds the corresponding :meth:`~Esibd.EsibdPlugins.Plugin.navToolBar` to the plugin :meth:`~Esibd.EsibdPlugins.Plugin.titleBar`.
    ...
    def getIcon(self):
        ...
        :return: Icon
        :rtype: :class:`~Esibd.EsibdCore.BetterIcon`
        """
    ...
    def makeIcon(self, file):
        """Returns an icon based on a filename. Looks for files in the :meth:`~Esibd.EsibdPlugins.Plugin.dependencyPath`.
    ...
    def getDefaultSettings(self):
        """Defines a dictionary of :meth:`~Esibd.EsibdCore.parameterDict` which specifies default settings for this plugin. 
        Overwrite or extend as needed to define specific settings that will be added to :ref:`sec:settings`.               

        :return: Settings dictionary
        :rtype: {:meth:`~Esibd.EsibdCore.parameterDict`}
        """
        ds = {}
...
class StaticDisplay(Plugin):
    """Displays :class:`~Esibd.EsibdPlugins.Device` data from file."""
...
class LiveDisplay(Plugin):
    """Displays :class:`~Esibd.EsibdPlugins.Device` data in real time."""
...
class Device(Plugin):
    """A :class:`~Esibd.EsibdPlugins.Device` manages access to parameters on a physical device, represented as a :class:`~Esibd.EsibdCore.Channel`."""

    version = 1.0
    name = 'Device' # overwrite after inheriting
    pluginType = PluginManager.TYPE.INPUTDEVICE
    """ :class:`Devices<Esibd.EsibdPlugins.Device>` are categorized as input or output devices.
    Overwrite with :attr:`~Esibd.EsibdCore.PluginManager.TYPE.OUTPUTDEVICE` after inheriting if applicable."""
    channelType = EsibdCore.Channel
    """Type of :class:`~Esibd.EsibdCore.Channel` used by the device. Overwrite by appropriate type in derived classes."""
    ...
    channels : [Channel]
    """List of :class:`channels<Esibd.EsibdCore.Channel>`."""
    useBackgrounds : bool
    ...
    def init(self):
        ...
        Device communication is initialized by the corresponding :class:`Esibd.EsibdCore.DeviceController`"""
        self.resetPlot()
    ...
    def loadConfiguration(self, file=None, default=False):
        """Loads :class:`~Esibd.EsibdCore.Channel` configuration from file.
    ...
    def loadValues(self, file = None):
        """Loads values only, instead of entire configuration, for :class:`channels<Esibd.EsibdCore.Channel>` matching in file and current configuraion.
    ...
    def apply(self, apply=False):
        """Applies :class:`~Esibd.EsibdCore.Channel` values to physical devices. Only used by input :class:`devices<Esibd.EsibdPlugins.Device>`.
    ...
    def plot(self):
        """Plots current values from all real :class:`channels<Esibd.EsibdCore.Channel>`."""
    ...
    def exportConfiguration(self, file=None, default=False):
        """Saves an .ini or .h5 file which contains the configuration for this :class:`~Esibd.EsibdPlugins.Device`.
        The .ini file can be easily edited manually with a text editor to add more :class:`channels<Esibd.EsibdCore.Channel>`."""
    ...
    def appendOutputData(self, f, default=False):
        """Appends :class:`~Esibd.EsibdPlugins.Device` data to hdf file."""
        fullRange = True
    ...
    def addChannel(self, item, index=None):
        """Maps dictionary to :class:`~Esibd.EsibdCore.Channel`."""
    ...
    def toggleRecording(self):
        """Toggle recoding of data in :class:`~Esibd.EsibdPlugins.LiveDisplay`."""
        if self.liveDisplayActive():
    ...
    def runDataThread(self, recording):
        ...
        This uses the current value of :class:`channels<Esibd.EsibdCore.Channel>` which is updated 
        independently by the corresponding :class:`~Esibd.EsibdCore.DeviceController`."""
        while recording():
...
class Scan(Plugin):
    """ Base class for :class:`scans<Esibd.EsibdPlugins.Scan>` and other measurement plugins.
    ...
    """Reference to :meth:`~Esibd.EsibdPlugins.DeviceManager.getChannelByName`."""
    file : Path
    ...
    """List of input :class:`meta channels<Esibd.EsibdCore.MetaChannel>`."""
    outputs : [EsibdCore.MetaChannel]
    """List of output :class:`meta channels<Esibd.EsibdCore.MetaChannel>`."""
    ...
    @property
    def finished(self):
        """True before and after scanning. Even when :attr:`~Esibd.EsibdPlugins.Scan.recording` is set to False
    ...
    def getOutputIndex(self):
        """Gets the index of the output channel corresponding to the currently selected display channel. See :attr:`~Esibd.EsibdPlugins.Scan.useDisplayChannel`.
        """
...
"""
    ...
    def getData(self, i, inout):
        ...
        :type inout: :attr:`~Esibd.EsibdCore.INOUT`
...
class SettingsManager(Plugin):
    """Bundles multiple :class:`settings<Esibd.EsibdCore.Setting>` into a single object to handle shared functionality."""
...
class DeviceManager(Plugin):
    ...
    def getChannelByName(self, name, inout=INOUT.BOTH):
        ...
        :type name: str
        :param inout: Type of channel, defaults to :attr:`~Esibd.EsibdCore.INOUT.BOTH`
        :type inout: :attr:`~Esibd.EsibdCore.INOUT`, optional
        :return: The requested channel.
        :rtype: :class:`~Esibd.EsibdCore.Channel`
        """
